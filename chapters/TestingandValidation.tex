...
vulnerabilities(ROP and JOP) or Frida\footnote{\url{https://www.frida.re} - \url[https://github.com/frida/frida]} which allows the user to inject snippets of code inside a Black Box binary. For these reasons the Gray Box technique can efficiently improve the coverage produced by the classic Black Box approach still without requiring access to the source code. Moreover this approach becomes very powerful if the source code is available as it can modify the program during the compilation phase to efficently trace the execution flow and the internal state. The information gathered with this method can then be used directly by the user or by another program to conduct further tests more efficiently.
